...
GameView::GameView() {
    background.setSize(1110 * m_config.resolution / 2, 1111 * m_config.resolution / 2);
    background.setTopLeftPosition(0, 0);
    background.setImage(ImageFileFormat::loadFrom(File("./img/Board.png")).rescaled(1110 * m_config.resolution / 2, 1111 * m_config.resolution / 2, Graphics::ResamplingQuality::highResamplingQuality));
    addAndMakeVisible(background);

    textures = Texture(m_config.resolution);
    ...
    for (auto &x: x_index) {
        x = (int) ((float) x * ((float) m_config.resolution / 2.0f));
    }
    for (auto &y: y_index) {
        y = (int) ((float) y * ((float) m_config.resolution / 2.0f));
    }
    auto default_image = textures.textures[Piece::NoneType];
    for (size_t i = 0; i < 9; ++i) {
        for (size_t j = 0; j < 10; ++j) {
            pieces[i][j].setSize(m_config.resolution * 50, m_config.resolution * 50);
    ...
    setSize(1110 * m_config.resolution / 2, 1111 * m_config.resolution / 2);
}
